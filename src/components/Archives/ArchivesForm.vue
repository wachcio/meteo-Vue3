  ...
  <div class="wrapper">
    <div class="archiveFormContainer">
      ...
      <form id="archiveForm">
        ...
        >
          <option
            v-for="(sensor, index) in sensorsNames"
            :key="index"
            :value="index"
          >
            {{ sensor }}
      ...
      <div v-if="JSONerror" id="result">
        <h2>Podano zły okres</h2>
      </div>
      <div v-if="isResponse && !JSONerror" id="result">
        <h2>Wyniki dla czujnika "{{ responseMax.sensorName }}" za okres {{ responsePeriod }}:</h2>

        <p
          v-show="windDirectionIndex !== sensorIndex"
        >
          Najwyższy odczyt: {{ responseMax.value }}{{ responseMax.unit }} ({{ responseMax.date }})
        </p>
        <p
          v-show="windDirectionIndex !== sensorIndex"
        >
          Najniższy odczyt: {{ responseMin.value }}{{ responseMin.unit }} ({{ responseMin.date }})
        </p>
        <p
          v-show="windDirectionIndex !== sensorIndex"
        >
          Średni odczyt: {{ responseAvg.value }}{{ responseAvg.unit }}
        </p>
        <p v-show="sensorIndex==rainIndex">
          Suma: {{ responseSum.value }}{{ responseMax.unit }}
        </p>
        <p
          v-show="sensorIndex==windDirectionIndex"
        >
          Dominującym kierunkiem wiatru był: {{ responseMax.value }}{{ responseMax.unit }}
        </p>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    formSend() {
      ...
      let result =
        `http://meteo.wachcio.pl/API/GetJSON.php?year=${
          this.year
        }&sensor=${
          this.sensorIndex}`;
