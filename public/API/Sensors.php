    ...
    public function getStatisticData($operation, $sensor, $hour, $day, $month, $year)
    {
        ...
        if ($sensorType == "Odczyty z Raspberry Pi" || $sensorType == "Odczyty ze studni") {
            ...
            if (($operation == "avg") || ($operation == "max") || ($operation == "min")) {
                if ($operation == "avg") {
                    $query = "SELECT " . $operation . "(`value`) AS `" . $operation . "` FROM  `" . $tableName . "` WHERE YEAR( `" . $dateColumn . "`) = " . $year;
                }
                if ($operation == "max" || $operation == "min") {
                    $query = "SELECT `value` AS `" . $operation . "`, `" . $dateColumn . "` FROM  `" . $tableName . "` WHERE YEAR( `" . $dateColumn . "`) = " . $year;
                }
                if ($month > 0 && $month < 13) {
                    $query = $query . " AND MONTH( `" . $dateColumn . "` ) = " . $month;
                } else {
                    $month = -1;
                }

                if ($day > 0 && $day < 31) {
                    $query = $query . " AND DAY( `" . $dateColumn . "` ) = " . $day;
                } else {
                    $day = -1;
                }

                if ($hour > -1 && $hour < 24) {
                    $query = $query . " AND HOUR( `" . $dateColumn . "` ) = " . $hour;
                } else {
                    $hour = -1;
                }
                if ($operation == "max") {
...
// $s=new Sensors;
// $s->monthSensors(8,2018);wachcio@u18:~/websites/meteo/API$
